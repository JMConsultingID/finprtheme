 ...
 */
class FinPr_Single_Product_Checkout {

    /**
     * Constructor: Register hooks and filters.
     */
    public function __construct() {
        add_action('init', [$this, 'finpr_setup_single_product_checkout_mode']);
        add_action('wp', [$this, 'finpr_remove_terms_and_conditions']);
    }

    /**
     * Setup single product checkout mode.
     */
    public function finpr_setup_single_product_checkout_mode() {
        // Disable add to cart messages.
        add_filter('wc_add_to_cart_message_html', '__return_false');

        // Remove previous product before adding a new one.
        //add_filter('woocommerce_add_cart_item_data', [$this, 'finpr_remove_previous_product'], 10, 2);

        // Empty the cart before adding a new product.
        add_filter('woocommerce_add_cart_item_data', [$this, 'finpr_empty_cart_before_adding_product']);

        // Redirect to checkout after adding product.
        add_filter('woocommerce_add_to_cart_redirect', [$this, 'finpr_redirect_to_checkout']);

        // Check for multiple products in the cart at checkout.
        //add_action('woocommerce_before_checkout_form', [$this, 'finpr_check_for_multiple_products']);

        // Disable order notes field.
        add_filter('woocommerce_enable_order_notes_field', '__return_false');

        // Change the order button text.
        add_filter('woocommerce_order_button_text', [$this, 'finpr_customize_order_button_text']);

        // Modify WooCommerce billing fields.
        add_filter('woocommerce_checkout_fields', [$this, 'finpr_modify_billing_fields']);

        // Hide specific countries on checkout.
        add_filter('woocommerce_countries', [$this, 'finpr_hide_countries_on_checkout'], 10, 1);
    }

    /**
     * Empty the cart before adding a new product.
     */
    public function finpr_empty_cart_before_adding_product($cart_item_data) {
     ...
     */
    public function finpr_remove_previous_product($cart_item_key, $product_id) {
     ...
     */
    public function finpr_redirect_to_checkout() {
     ...
     */
    public function finpr_check_for_multiple_products() {
     ...
     */
    public function finpr_refresh_cart_keep_last_product() {
     ...
     */
    public function finpr_customize_order_button_text() {
     ...
     */
    public function finpr_modify_billing_fields($fields) {
     ...
     */
    public function finpr_remove_terms_and_conditions() {
     ...
     */
    public function finpr_hide_countries_on_checkout($countries) {
